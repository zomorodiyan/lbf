...
# see those functions' own comments, 2026-08-03; render_xray's is a fixed
# ray-tracing grid; render_transverse's matplotlib bbox is stable in
# practice since its always-drawn frame/ghost-frame outlines dominate it
# regardless of melt content), so stacking is a straight single pass with
# no separate size-normalization step needed.
#
...
for t in "${TIMES[@]}"; do
  ...
  # no per-panel cropping. W is the same on every iteration (every view's
  # own dimensions are fixed, see the header comment above) -- kept as a
  # per-iteration ffprobe computation anyway rather than hoisted out, since
  # it's cheap and self-verifying if that ever stops being true.
