...
#
# Usage:
#   bash results/scripts/measure_melt_vapor_depth.sh <case> [extra args passed through
...
#
# Examples:
#   bash results/scripts/measure_melt_vapor_depth.sh 81
#   bash results/scripts/measure_melt_vapor_depth.sh 84 --steady-frac 0.4 --tmelt solidus
#
...
#   results/melt_pool_measurements/<prefix>_melt_vapor_depth.csv
#   results/melt_pool_measurements/<prefix>_melt_vapor_depth.png
set -euo pipefail
cd ~/lbf3

if [ $# -lt 1 ]; then
  echo "Usage: bash results/scripts/measure_melt_vapor_depth.sh <case> [extra args]"
...
docker run --rm --user "$(id -u):$(id -g)" -e PYTHONUNBUFFERED=1 -v "$(pwd)":/workspace \
  ...
  "/workspace/$FOAM_FILE" \
  "/workspace/results/melt_pool_measurements/${PREFIX}_melt_vapor_depth.csv" \
  "/workspace/results/melt_pool_measurements/${PREFIX}_melt_vapor_depth.png" \
  "$@"

echo "DONE: results/melt_pool_measurements/${PREFIX}_melt_vapor_depth.csv, results/melt_pool_measurements/${PREFIX}_melt_vapor_depth.png"
