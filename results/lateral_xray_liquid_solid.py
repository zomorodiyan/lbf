...
# "the bottom of the melt pool" as seen from this lateral view -- alongside
# the gas/metal (vapor depression) boundary, without baking the liquid/solid
# distinction into the displayed grayscale itself (MU_SOLID == MU_LIQUID for
# the main image; see below for why).
#
...
#     /workspace/results/lateral_xray_liquid_solid.py <case.foam> <time> <output.png>
#
...
if len(sys.argv) != 4:
    print("Usage: pvpython lateral_xray_liquid_solid.py <case.foam> <time> <output.png>")
    sys.exit(1)
...
# Boundary lines, both from a continuous-attenuation-ceiling test (see v3 in
# the header) rather than a boolean presence flag:
#   - vapor depression bottom: from the *main* (equal-mu) mu_integral -- its
#     ceiling (100% metal, whether liquid or solid, over X_WIDTH) is
#     MU_SOLID*X_WIDTH (== MU_LIQUID*X_WIDTH since they're equal here). Any
#     gas along the ray strictly lowers mu_integral below that.
#   - melt pool bottom: needs the *auxiliary* mu_integral_aux (distinct
#     MU_SOLID_AUX/MU_LIQUID_AUX) -- its ceiling (100% solid, no liquid, no
#     gas) is MU_SOLID_AUX*X_WIDTH. Any liquid *or* gas lowers it below that,
#     which is fine: liquid implies not-fully-solid either way, and any
#     column with gas already fails the (stricter) vapor-depression ceiling
#     too, so this curve is always at least as deep as that one.
#
# The two lines use different tolerances now, tuned separately:
#   - white (vapor depression): THRESHOLD_FRAC_GAS relaxes the ceiling to 78%
#     -- a ray needs at least ~22% of its path length to be gas before the
#     line marks it, so trace amounts (a sliver of a spatter droplet, mesh
#     noise) don't trigger it.
#   - blue (melt pool): THRESHOLD_FRAC_MELT relaxes the ceiling to 98% -- a
#     ray needs at least ~2% of its path length to be liquid/gas before the
#     line marks it (also windowed to near the laser -- see below -- so this
#     only needs to reject fine mesh-sliver noise, not filter distant
#     spurious detections).
THRESHOLD_FRAC_GAS = 0.78
THRESHOLD_FRAC_MELT = 0.98
_X_WIDTH_MM = X_WIDTH * 1e3
_gas_ceiling = MU_SOLID * _X_WIDTH_MM
...
MELT_FRONT_OFFSET = 0.05e-3        # 0.05mm ahead of the laser spot
MELT_WINDOW_BEHIND_FRONT = 0.6e-3  # 0.6mm

# White line z-window: fixed lower bound (0.5mm -- the track only starts
# being interesting there) up to whichever is larger of a fixed 2.5mm or the
# laser's own current position + a small forward margin. The max(...) means
# this window normally just shows the fixed 0.5-2.5mm stretch, but once the
# laser scans past ~2.4mm it keeps following the laser instead of clipping
# the vapor depression's current position out of view.
...
gas_bottom = _threshold_bottom(mu_integral_avg, _gas_ceiling, ys, THRESHOLD_FRAC_GAS)
...
ax.plot(zs * 1e3, gas_bottom * 1e3, linestyle=':', color='white', linewidth=1.2,
        label='vapor depression bottom (gas/metal)', zorder=3)
ax.set_xlabel('z - scan direction (mm)')
ax.set_ylabel('y - depth from surface (mm)')
ax.set_title(f'lateral X-ray-style projection (liquid/solid), t={time_value*1e6:.0f} us')
fig.tight_layout()
